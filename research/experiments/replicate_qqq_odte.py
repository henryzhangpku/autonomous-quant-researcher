"""Replication: the near-money 0DTE put credit, on tape it has never seen.
...
OUT_DIR = REPOSITORY_ROOT / ".research" / "sweeps" / "qqq-odte-replication-v1"
SNAPSHOT_DIR = REPOSITORY_ROOT / ".research" / "data" / "odte" / "qqq-nearmoney"
...
def build_snapshot(start: str, end: str) -> dict:
    ...
    while cursor < end_dt:
        ...
        spot_bars.extend(provider.bars(asset_class="equity", symbols=("QQQ",),
        ...
        for strike in (short_k, long_k):
            try:
                snap = provider.bars(asset_class="option",
                                     symbols=(occ_symbol("QQQ", day, strike),),
    ...
    manifest = {
        "underlying": "QQQ", "structure": "put_credit_spread near-money (pct-defined)",
...
def main() -> int:
    ...
    for label, cost_bps in COST_LEVELS.items():
        spec_payload = {
            "name": f"qqq-odte-replication-{label}",
            "universe": ["QQQ-0DTE-PCS-NM"],
            "splits": splits,

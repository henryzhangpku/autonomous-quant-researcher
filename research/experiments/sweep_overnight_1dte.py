...
OUT_DIR = REPOSITORY_ROOT / ".research" / "sweeps" / "overnight-1dte-v1"

ENTRY_MINUTES = tuple(range(135, 901, 45)) + (915,)
STRUCTURES = ("spx_put_credit_spread", "spx_call_debit_spread")
WIDTHS = (20.0, 25.0)
...
def run_cell(candidate: Path, split: str, exit_mode: str) -> dict:
    completed = subprocess.run(
        [sys.executable, "-m", "research.validators.overnight_spx_v2",
         ...
         "--exit", exit_mode, "--horizon", "next_session"],
...
def main() -> int:
    ...
    summary = {
        "question": "does the hold-through 1DTE horizon (enter tonight, settle TOMORROW 16:00 ET) pay?",

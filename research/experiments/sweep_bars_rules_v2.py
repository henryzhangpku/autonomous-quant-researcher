"""Bars campaign 2: the same grid, priced against the market.

Campaign 1 promoted nothing, but it named its own defect: all four discovery
gate-passers were `ret_20d`, and BOTH directions passed long — buy after a
20-day decline and buy after a 20-day advance. A condition whose opposite
passes equally carries no information; what passed was the long side of a
discovery window (2021-08 to 2024-08) that had one direction in it.

Campaign 2 changes exactly one thing: the payoff is benchmark-relative. Each
symbol's forward return has SPY's same-session return subtracted, so drift
earns nothing and a rule must beat the market on the sessions it selects.
Everything else — universe, grid, thresholds, costs, gates, promotion rule —
is identical, so any difference in outcome is attributable to the change.
...
OUT_DIR = REPOSITORY_ROOT / ".research" / "sweeps" / "bars-rules-v2"
SNAPSHOT_DIR = REPOSITORY_ROOT / ".research" / "data" / "bars" / "bars-campaign-2"
